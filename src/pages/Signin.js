...
const Signin = () => {
  ...
  const handleSubmit = async (values, { setSubmitting, setErrors }) => {
    setLoading(true);
    try {
      const response = await axios.post(apiBase + "/auth/login", values);
      ...
      if(response.data.user.role === "ADMIN"){
        ...
        setTimeout(() => navigate("/project"), 1000); // Navigate after 1 second
      }
      // setTimeout(() => navigate("/forgeviewer"), 1000); // Navigate after 1 second
    } catch (error) {
